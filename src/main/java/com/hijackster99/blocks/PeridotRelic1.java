...
public class PeridotRelic1 extends ARBlocks{

	static DirectionProperty property = DirectionProperty.create("facing", Direction.values());

	public PeridotRelic1(String registryName, Material materialIn, float hardnessIn, float resistanceIn, ToolType harvestTool, int miningLevel) {
	...
	public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos, boolean isMoving) {
		...
		if(!worldIn.getBlockState(pos.subtract(state.get(property).getDirectionVec())).getBlock().equals(ARBlocks.RITUAL_STONE)) {
	...
	public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if(player.getHeldItem(handIn) == null || player.getHeldItem(handIn).isEmpty() && player.isSneaking()) {
			if(worldIn.getBlockState(stonePos).getBlock().equals(ARBlocks.RITUAL_STONE)) {
				worldIn.removeBlock(pos, false);
				worldIn.setBlockState(stonePos, ARBlocks.RITUAL_STONE_1.getDefaultState());
			}
			return true;
		}
		return false;
