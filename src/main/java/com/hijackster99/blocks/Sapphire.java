...
public class Sapphire extends ARBlocks {

static DirectionProperty property = DirectionProperty.create("facing", Direction.values());
	
	public Sapphire(String registryName, Material materialIn, float hardnessIn, float resistanceIn, ToolType harvestTool, int miningLevel) {
	...
	public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos, boolean isMoving) {
		...
		if(!worldIn.getBlockState(pos.subtract(state.get(property).getDirectionVec())).getBlock().equals(Blocks.BEDROCK)) {
	...
	public VoxelShape getCollisionShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
		if(state.get(property).equals(Direction.UP))
			return VoxelShapes.create(new AxisAlignedBB(0.3125, 0, 0.3125, 0.6875, 0.0625, 0.6875));
		else if(state.get(property).equals(Direction.DOWN))
			return VoxelShapes.create(new AxisAlignedBB(0.3125, 0.9375, 0.3125, 0.6875, 1, 0.6875));
		else if(state.get(property).equals(Direction.NORTH))
			return VoxelShapes.create(new AxisAlignedBB(0.3125, 0.3215, 0.9375, 0.6875, 0.6875, 1));
		else if(state.get(property).equals(Direction.SOUTH))
			return VoxelShapes.create(new AxisAlignedBB(0.3125, 0.3125, 0, 0.6875, 0.6875, 0.0625));
		else if(state.get(property).equals(Direction.EAST))
			return VoxelShapes.create(new AxisAlignedBB(0, 0.3125, 0.3125, 0.0625, 0.6875, 0.6875));
		else
			return VoxelShapes.create(new AxisAlignedBB(0.9375, 0.3125, 0.3125, 1, 0.6875, 0.6875));
	}
	...
	public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		if(player.getHeldItem(handIn) == null || player.getHeldItem(handIn).isEmpty() && pos.getY() <= 2) {
			worldIn.setBlockState(pos, ARBlocks.SAPPHIRE_RELIC.getState(state.get(property)));
			return true;
		}
		return false;
	}
