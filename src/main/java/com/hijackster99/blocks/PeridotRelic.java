...
public class PeridotRelic extends ARBlocks {

	static DirectionProperty property = DirectionProperty.create("facing", Direction.values());
	
	public PeridotRelic(String registryName, Material materialIn, float hardnessIn, float resistanceIn, ToolType harvestTool, int miningLevel) {
	...
	public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos, boolean isMoving) {
		...
		if(!worldIn.getBlockState(pos.subtract(state.get(property).getDirectionVec())).getBlock().equals(Blocks.BEDROCK) && !worldIn.getBlockState(pos.subtract(state.get(property).getDirectionVec())).getBlock().equals(Blocks.STONE) && !worldIn.getBlockState(pos.subtract(state.get(property).getDirectionVec())).getBlock().equals(ARBlocks.INFUSED_STONE)) {
	...
	public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if(player.getHeldItem(handIn) == null || player.getHeldItem(handIn).isEmpty() && player.isSneaking() && stonePos.getY() <= 2) {
			...
			}else if(worldIn.getBlockState(stonePos).getBlock().equals(ARBlocks.INFUSED_STONE)) {
				worldIn.removeBlock(pos, false);
				worldIn.setBlockState(stonePos, ARBlocks.RITUAL_STONE.getDefaultState());
			}
			return true;
		}
		return false;
